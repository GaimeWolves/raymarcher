/*
* COPYRIGHT stevenBorisko 2021 https://github.com/stevenBorisko/color
*/
...
namespace color
{
    ...
    */
    struct hsl
    {
        ...
        {
            if (s == 0 && h.has_value())
                throw std::runtime_error{"Defined hue with zero saturation"};
            if (s != 0 && !h.has_value())
                throw std::runtime_error{"Undefined hue with non-zero saturation"};
            if (h.has_value() && (h.value() < 0 || 360 <= h.value()))
                throw std::runtime_error{"Out of range hue"};
            if (s < 0 || 1 < s)
                throw std::runtime_error{"Out of range saturation"};
            if (l < 0 || 1 < l)
                throw std::runtime_error{"Out of range brightness"};
        }
    };
    ...
    */
    struct rgb
    {
        ...
        {
            if (r < 0 || 1 < r)
                throw std::runtime_error{"Out of range red"};
            if (g < 0 || 1 < g)
                throw std::runtime_error{"Out of range green"};
            if (b < 0 || 1 < b)
                throw std::runtime_error{"Out of range blue"};
        }
    };
    ...
    namespace convert
    {
        ...
        static auto to_rgb(color::hsl const &hsl) -> color::rgb
        {
            ...
            case 5:
                return {v, min, mid2};
            default:
                throw std::runtime_error{"failed to convert hsl to rgb"};
            }
        }
        ...
        static auto to_rgb(color::hsv const &hsv) -> color::rgb
        {
            ...
            case 5:
                return {hsv.v, p, q};
            default:
                throw std::runtime_error{"failed to convert hsv to rgb"};
            }
        }
        ...
        static auto to_hsv(color::rgb const &rgb) -> color::hsv
        {
            ...
            throw std::runtime_error{"failed to convert rgb to hsv"};
        }
